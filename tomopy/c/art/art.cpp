...
extern "C"
{
    Art* create(float *data, float *theta, float *center, 
                int *num_projections, int *num_slices, 
                int *num_pixels, int *num_grid, int *num_air) {
        return new Art(data, theta, center, 
...
Art::Art(float *data, float *theta, float *center, 
         int *num_projections, int *num_slices, 
         int *num_pixels, int *num_grid, int *num_air) :
data_(data),
theta_(theta),
    ...
    for (t = 0; t < *iters; t++) {
        ...
        for (q = 0; q < num_projections_; q++) {
            ...
            for (m = 0; m < padded_data_size; m++) {
                ...
                j = 0;
                k = 0;
                if ((theta_[q] >= 0 && theta_[q] < PI/2) || 
                    (theta_[q] >= PI && theta_[q] < 3*PI/2)) {
                ...
                for (k = 0; k < num_slices_; k++) {
                    ...
                    for (n = 0; n < len-1; n++) {
                        ii = indi[n] + k * (num_grid_ * num_grid_);
                        recon[ii] += (padded_data[io] - simdata) / a2 * leng[n];
                    }
                }
            }
        }
